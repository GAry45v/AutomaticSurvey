...
class outlineWriter():
    ...
    def draft_outline(self, topic, requirement, reference_num = 600, chunk_size = 30000, section_num = 6):
        # Get database
        ...
        # new param 'requirement'
        outlines = self.generate_rough_outlines(topic=topic, requirement = requirement, papers_chunks = abs_chunks, titles_chunks = titles_chunks, section_num=section_num)
        
        # to get the final outline
        section_outline = self.merge_outlines(topic=topic, requirement = requirement, outlines=outlines)

        # generate subsection-level outline
        subsection_outlines = self.generate_subsection_outlines(topic=topic, requirement = requirement, section_outline= section_outline, rag_num= 50)
        
        merged_outline = self.process_outlines(section_outline, subsection_outlines)
        
        # edit final outline
        final_outline = self.edit_final_outline(requirement, merged_outline)
    ...
    def generate_rough_outlines(self, topic, requirement, papers_chunks, titles_chunks, section_num = 8):
        ...
        self.output_token_usage += self.token_counter.num_tokens_from_list_string([o for o in outlines if o is not None])
        return outlines
    
    def merge_outlines(self, topic, requirement, outlines):
        '''
    ...
    def generate_subsection_outlines(self, topic, requirement, section_outline, rag_num):
        '''
        You are an expert in artificial intelligence who wants to write a overall survey about [TOPIC].\n\
        ...
        for section_name, section_description in zip(survey_sections, survey_section_descriptions):
            ...
            prompt = self.__generate_prompt(SUBSECTION_OUTLINE_PROMPT, paras={'OVERALL OUTLINE': section_outline,'SECTION NAME': section_name,\
                                                                          'SECTION DESCRIPTION':section_description,'TOPIC':topic, 'REQUIREMENT': requirement, 'PAPER LIST':paper_texts})
            prompts.append(prompt)
        self.input_token_usage += self.token_counter.num_tokens_from_list_string(prompts)

        sub_outlines = self.api_model.batch_chat(prompts, temperature=1)

        self.output_token_usage += self.token_counter.num_tokens_from_list_string([s for s in sub_outlines if s is not None])
        return sub_outlines

    def edit_final_outline(self, requirement, outline):
        '''
        You are an expert in artificial intelligence who wants to write a overall survey about [TOPIC].\n\
        ...
        prompt = self.__generate_prompt(EDIT_FINAL_OUTLINE_PROMPT, paras={'REQUIREMENT': requirement, 'OVERALL OUTLINE': outline})
        self.input_token_usage += self.token_counter.num_tokens_from_string(prompt)
        outline = self.api_model.chat(prompt, temperature=1)
        self.output_token_usage += self.token_counter.num_tokens_from_string(outline)
